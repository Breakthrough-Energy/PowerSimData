...
def test_angular_distance():
    ...
    assert_almost_equal(angular_distance(
        [sqrt(2)/2, sqrt(2)/2, 0.0], [0.0, 0, -1.0]), 90)
    # 45 deg longitude to 60 deg longitude
    assert_almost_equal(angular_distance(
        [sqrt(2)/2, sqrt(2)/2, 0.0], [1/2, sqrt(3)/2, 0.0]), 15)


def test_find_closest_neighbor():
    point = (45, 45)
    neighbors = [[0, 45], [10, 50], [40, 40], [-120, -60], [44.75, 45.1],
                 [-270, 5], [43, 46], [320, 45], [45, 44], [44, 45],
                 [44.5, 45.5]]

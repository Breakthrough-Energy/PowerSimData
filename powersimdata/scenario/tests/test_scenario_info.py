...
mock_demand = pd.concat([
    mock_pg[[plant_id for plant_id, zone_name in
             zip(mock_plant['plant_id'], mock_plant['zone_name'])
             if zone_name == 'zone1']].sum(axis=1).to_frame(),
    mock_pg[[plant_id for plant_id, zone_name in
             zip(mock_plant['plant_id'], mock_plant['zone_name'])
             if zone_name == 'zone2']].sum(axis=1).to_frame()
], axis=1)
...
class TestScenarioInfo(unittest.TestCase):
    ...
    def test_get_demand(self):
        ...
        assert self.scenario_info.get_demand('zone2', start_time, end_time) \
    ...
    def test_get_generation(self):
        assert self.scenario_info.\
            get_generation('solar', 'zone1', start_time, end_time) == \
            mock_pg[solar_plant_id].sum().sum()
        assert self.scenario_info.\
            get_generation('wind', 'zone2', start_time, end_time) == \
            mock_pg[wind_plant_id].sum().sum()
        assert self.scenario_info.\
            get_generation('ng', 'all', start_time, end_time) == \
            mock_pg[ng_plant_id].sum().sum()
        assert self.scenario_info.\
            get_generation('coal', 'all', start_time, end_time) == \
            mock_pg[coal_plant_id].sum().sum()
        assert self.scenario_info.\
            get_generation('dfo', 'all', start_time, end_time) == \
            mock_pg[dfo_plant_id].sum().sum()
        assert self.scenario_info.\
            get_generation('hydro', 'zone1', start_time, end_time) == \
            mock_pg[hydro_plant_id].sum().sum()

    def test_get_curtailment(self):
        assert self.scenario_info.\
            get_curtailment('solar', 'all', start_time, end_time) == \
            round(1 - (mock_pg[solar_plant_id].sum().sum()
                       / mock_solar.sum().sum()), 4)
        assert self.scenario_info.\
            get_curtailment('wind', 'all', start_time, end_time) == \
            round(1 - (mock_pg[wind_plant_id].sum().sum()
                       / mock_wind.sum().sum()), 4)
        assert self.scenario_info.\
            get_curtailment('hydro', 'all', start_time, end_time) == \
            round(1 - (mock_pg[hydro_plant_id].sum().sum()
                       / mock_hydro.sum().sum()), 4)

    def test_get_profile_resource(self):
        assert self.scenario_info.\
            get_profile_resource('solar', 'all', start_time, end_time)\
            == mock_solar.sum().sum()
        assert self.scenario_info.\
            get_profile_resource('wind', 'all', start_time, end_time)\
            == mock_wind.sum().sum()
        assert self.scenario_info.\
            get_profile_resource('hydro', 'all', start_time, end_time)\
            == mock_hydro.sum().sum()

    def test_get_capacity_factor(self):
        assert self.scenario_info.\
            get_capacity_factor('solar', 'all', start_time, end_time) \
            == round(mock_pg[solar_plant_id].sum().sum() /
                     (50 * period_num), 4)
        assert self.scenario_info.\
            get_capacity_factor('wind', 'all', start_time, end_time) \
            == round(mock_pg[wind_plant_id].sum().sum() /
                     (200 * period_num), 4)
        assert self.scenario_info.\
            get_capacity_factor('ng', 'all', start_time, end_time) \
            == round(mock_pg[ng_plant_id].sum().sum() /
                     (80 * period_num), 4)
        assert self.scenario_info.\
            get_capacity_factor('coal', 'all', start_time, end_time) \
            == round(mock_pg[coal_plant_id].sum().sum() /
                     (100 * period_num), 4)
        assert self.scenario_info.\
            get_capacity_factor('dfo', 'all', start_time, end_time) \
            == round(mock_pg[dfo_plant_id].sum().sum() /
                     (120 * period_num), 4)
        assert self.scenario_info.\
            get_capacity_factor('hydro', 'all', start_time, end_time) \
            == round(mock_pg[hydro_plant_id].sum().sum() /
                     (220 * period_num), 4)

    def test_get_no_congest_capacity_factor(self):
        assert self.scenario_info.\
            get_no_congest_capacity_factor('solar', 'all',
                                           start_time, end_time) == \
            round(mock_solar.sum().sum() / (50 * period_num), 4)
        assert self.scenario_info.\
            get_no_congest_capacity_factor('wind', 'all',
                                           start_time, end_time) == \
            round(mock_wind.sum().sum() / (200 * period_num), 4)
        assert self.scenario_info.\
            get_no_congest_capacity_factor('hydro', 'all',
                                           start_time, end_time) == \

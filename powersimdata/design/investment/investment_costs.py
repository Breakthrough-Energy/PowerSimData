...
def _calculate_ac_inv_costs(grid_new, year):
    ...
    #     bus_mult = pd.read_csv(os.path.join(DIR, "buses_NEEMregion.csv"))
    #     bus_mult.melt(id_vars=['kV_cost', 'MW'], var_name='region',val_name = 'mult',inplace=True)

    #     lines= lines.merge(bus_mult,on_left='to_bus_id',on_right='bus_id',how='inner')
    #     lines.rename(columns={"mult": "mult_to"},inplace=True)
    #     lines= lines.merge(bus_mult,on_left='from_bus_id',on_right='bus_id',how='inner')
    #     lines.rename(columns={"mult": "mult_from"},inplace=True)

    #     #take average between 2
    #     lines.loc[:,'mult'] = (lines['mult_to'] + lines['mult_from'])/2.0

    #     print(lines[lines['MW'] == 500])

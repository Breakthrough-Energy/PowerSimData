...
def _calculate_ac_inv_costs(grid_new, year):
    ...
    bus_mult = pd.read_csv(os.path.join(DIR, "LineRegMult.csv"))
    bus_mult = bus_mult.melt(id_vars=['kV_cost', 'MW'], var_name='name_abbr', value_name='mult')

    lines = lines.merge(bus_reg,left_on='to_bus_id',right_on='bus_id',how='inner')
    lines = lines.merge(bus_mult, on=['name_abbr', 'kV_cost', 'MW'], how="left")

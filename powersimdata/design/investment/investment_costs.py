...
def _calculate_ac_inv_costs(grid_new, year):
    ...
    def select_MW(x, cost_df):
        ...
        tmp = tmp[~tmp['MW'].isna()]
        if len(tmp) == 0:
            print('type MW', type(x))
            print('type MW cost_df', cost_df)
            print('empty!')
    ...
    # import data
    ac_cost = pd.read_csv(os.path.join(DIR, "LineBase.csv"))
    ac_reg_mult = pd.read_csv(os.path.join(DIR, "LineRegMult.csv"))
    xfmr_cost = pd.read_csv(os.path.join(DIR, "Transformers.csv"))

    # map line kV
    bus = grid_new.bus
    branch = grid_new.branch
    branch.loc[:, 'kV'] = branch.apply(lambda x: bus.loc[x.from_bus_id, 'baseKV'], axis=1)

    # separate transformers and lines
    transformers = branch[branch['branch_device_type'].isin(['Transformer', 'TransformerWinding'])].copy()
    lines = branch[~branch['branch_device_type'].isin(['Transformer', 'TransformerWinding'])].copy()
    ...
    bus_reg = pd.read_csv(os.path.join(DIR, "buses_NEEMregion.csv"),index_col="bus_id")
    ...
    bus_fix_index2 = bus_mask[~np.isclose(bus_mask.lat_x, bus_mask.lat_y) |
    ...
    print('bus_fix', bus_fix)
    bus_fix = bus_to_NEEM_reg(bus_fix, DIR)
    bus.loc[bus.index.isin(bus_fix.index), ["name_abbr", "lat", "lon"]] = bus_fix[["name_abbr", "lat", "lon"]]
    print('bus', bus)
    bus_reg.drop(["lat","lon"],axis=1,inplace=True)

    #map region multipliers onto lines
    ac_reg_mult = ac_reg_mult.melt(id_vars=['kV_cost', 'MW'], var_name='name_abbr', value_name='mult')
    ...
    print(lines[['rateA','reg_from','reg_to','mult_to','mult_from','mult','lengthMi','MWmi','Cost']])
...
def _calculate_gen_inv_costs(grid_new, year,cost_case):
    ...
    def load_cost(file_name,year,cost_case,DIR):
        ...
        :param str cost_case: the ATB cost case of data ['Moderate': mid cost case,'Conservative': generally higher costs,'Advanced': generally lower costs]
        :param str DIR: the Data directory.
    ...
    plants = plants[~plants.type.isin(['dfo', 'other'])]  # drop these technologies, no cost data
    ...
    plants.loc[:,'r'] = ''
    rs_tech = ['wind','wind_offshore','csp'] #wind regions (rs) (apply to wind and csp)
    plants.loc[plants['type'].isin(rs_tech),'r'] = plants.loc[plants['type'].isin(rs_tech),'rs']
    rb_tech =['solar','storage','nuclear','coal','ng','hydro','geothermal']  #BA regions (rb) (apply to rest of techs)

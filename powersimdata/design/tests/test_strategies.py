...
def test_independent_capacity_strategy_pacific_external_6():
    ...
    target = TargetManager("Pacific", 0.25, "renewables", 200000 * 1000, 14000 * 1000)
    target.set_allowed_resources(["solar", "wind", "geo"])
    target.add_resource(solar)
    target.add_resource(wind)
    target.add_resource(geo)
...
def test_collaborative_capacity_strategy():
    ...
    assert collab.targets["Pacific"].resources[
        "solar"
    ].prev_capacity * solar_scaling == approx(3700 + 9203.75)
    assert collab.targets["Pacific"].resources[
        "wind"
    ].prev_capacity * wind_scaling == approx(3600 + 8955)
    assert collab.targets["Atlantic"].resources[
        "solar"
    ].prev_capacity * solar_scaling == approx(4200 + 10447.5)
    assert collab.targets["Atlantic"].resources[
        "wind"
    ].prev_capacity * wind_scaling == approx(4100 + 10198.75)
...
def test_collaborative_capacity_strategy_overgeneration():
    ...
    assert collab.targets["Pacific"].resources[
        "solar"
    ].prev_capacity * solar_scaling == approx(3700 + 4578.75)
    assert collab.targets["Pacific"].resources[
        "wind"
    ].prev_capacity * wind_scaling == approx(3600 + 4455)
    assert collab.targets["Atlantic"].resources[
        "solar"
    ].prev_capacity * solar_scaling == approx(4200 + 5197.5)
    assert collab.targets["Atlantic"].resources[
        "wind"
    ].prev_capacity * wind_scaling == approx(4100 + 5073.75)
...
def test_collaborative_capacity_strategy_collab_curtailment():
    ...
    assert collab.targets["Pacific"].resources[
        "solar"
    ].prev_capacity * solar_scaling == approx(3700 + 10269.18)
    assert collab.targets["Pacific"].resources[
        "wind"
    ].prev_capacity * wind_scaling == approx(3600 + 9991.63)
    assert collab.targets["Atlantic"].resources[
        "solar"
    ].prev_capacity * solar_scaling == approx(4200 + 11656.9)
    assert collab.targets["Atlantic"].resources[
        "wind"
    ].prev_capacity * wind_scaling == approx(4100 + 11379.36)
...
def test_adding_addl_curtailment():
    ...
    independent_strategy_manager.set_addl_curtailment(
        {"Pacific": {"solar": 0.2, "wind": 0.5}}
    )

    assert (
        independent_strategy_manager.targets["Pacific"]
        .resources["solar"]
        .addl_curtailment
        == 0.2
    )
    ...
    assert (
        independent_strategy_manager.targets["Atlantic"]
        .resources["solar"]
        .addl_curtailment
        == 0
    )

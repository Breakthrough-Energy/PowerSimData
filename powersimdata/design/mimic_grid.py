def _calculate_common_zone_factors(base_plant, ref_plant, plant_scaling,
                                   epsilon=1e-3):
    ...
    grouping = plant_scaling[nonzero].groupby(
        [base_plant.type, base_plant.zone_id])
...
def mimic_generation_capacity(base_grid, ref_grid):
    ...
    change_table = _calculate_common_zone_factors(
        base_plant, ref_plant, plant_scaling)
    return change_table

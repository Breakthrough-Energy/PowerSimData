...
class TransformGrid(object):
    ...
    def _add_branch(self):
        ...
        for entry in self.ct["new_branch"]:
            ...
            new_branch["x"] = x
            self.grid.branch = self.grid.branch.append(new_branch, ignore_index=True)
    ...
    def _add_dcline(self):
        ...
        for entry in self.ct["new_dcline"]:
            ...
            self.grid.dcline = self.grid.dcline.append(new_dcline, ignore_index=True)
...
class ScaleProfile(object):
    ...
    def get_power_output(self, resource):
        ...
        if bool(self.ct) and bool(self.scale_keys[resource] & set(self.ct.keys())):

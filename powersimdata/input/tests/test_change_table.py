...
def test_add_thermal_plant_missing_key_c0(capsys):
    try:
        capsys.readouterr()
        new_plant = [{'type': 'ng', 'bus_id': 100, 'Pmax': 75, 'c1': 9,
                      'c2': 0.25}]
        ct.add_plant(new_plant)
        cap = capsys.readouterr()
...
def test_add_thermal_plant_missing_key_c1(capsys):
    try:
        capsys.readouterr()
        new_plant = [{'type': 'ng', 'bus_id': 100, 'Pmax': 75, 'c0': 1500,
                      'c2': 1}]
        ct.add_plant(new_plant)
        cap = capsys.readouterr()
...
def test_add_plant_wrong_bus(capsys):
    try:
        capsys.readouterr()
        new_plant = [
            {'type': 'nuclear', 'bus_id': 300, 'Pmin': 500, 'Pmax': 5000,
             'c0': 1, 'c1': 2, 'c2': 3},
            {'type': 'coal', 'bus_id': 5000000, 'Pmax': 200,
             'c0': 1, 'c1': 2, 'c2': 3}]
        ct.add_plant(new_plant)
        cap = capsys.readouterr()
...
def test_add_thermal_plant_wrong_coefficients(capsys):
    try:
        capsys.readouterr()
        new_plant = [
            {'type': 'ng', 'bus_id': 300, 'Pmin': 0, 'Pmax': 500,
             'c0': -800, 'c1': 30, 'c2': 0.0025}]
        ct.add_plant(new_plant)
        cap = capsys.readouterr()
...
def test_add_plant_negative_pmax(capsys):
    try:
        capsys.readouterr()
        new_plant = [{'type': 'dfo', 'bus_id': 300, 'Pmax': -10,
                      'c0': 1, 'c1': 2, 'c2': 0.3}]
        ct.add_plant(new_plant)
        cap = capsys.readouterr()
...
def test_add_plant_negative_pmin(capsys):
    try:
        capsys.readouterr()
        new_plant = [
            {'type': 'dfo', 'bus_id': 300, 'Pmax': 10,
             'c0': 100, 'c1': 2, 'c2': 0.1},
            {'type': 'geothermal', 'bus_id': 3001001, 'Pmin': -1, 'Pmax': 20,
             'c0': 10, 'c1': 5, 'c2': 1}]
        ct.add_plant(new_plant)
        cap = capsys.readouterr()
...
def test_add_plant_pmin_pmax_relationship(capsys):
    try:
        capsys.readouterr()
        new_plant = [
            {'type': 'biomass', 'bus_id': 13802, 'Pmin': 30, 'Pmax': 20,
             'c0': 30, 'c1': 15, 'c2': 0.1}]
        ct.add_plant(new_plant)
        cap = capsys.readouterr()
...
def test_add_renewable_plant_check_neighbor_is_added():
    try:
        new_plant = [
            ...
            {'type': 'coal', 'bus_id': 9, 'Pmax': 120,
             'c0': 1000, 'c1': 500, 'c2': 0.3},
            {'type': 'wind_offshore', 'bus_id': 13802, 'Pmax': 175}]
        ct.add_plant(new_plant)

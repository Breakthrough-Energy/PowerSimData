...
#### I. Visualizing Generator Supply Curves

Supply curves for each generator type in each interconnect or load zone can be created by 
first accessing the plant and generator cost data associated with a specified Grid object. 
This data is accessed using the following:

```
supply_df = powersimdata.design.generation.cost_curves.get_supply_data(grid)
```
where `grid` is a Grid object.
...
to which the generator belongs. To obtain the supply curve for a particular fuel type and 
area, the DataFrame is passed into the following:

```
P, F = powersimdata.design.generation.cost_curves.build_supply_curve(supply_df, area, type)
```
where `area` is a string describing an appropriate interconnect or load zone and `type` is a 
string describing an appropriate fuel type.
